	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	      ...
	      switch(user_choice)
			...
			case 1: 
		       ...
		       response.sendRedirect("TopLanguages.html");
				break;
		case 2: 
		...
		case 4:
			...
			response.sendRedirect("BackgroundColors.html");
			break;
		case 5:
			UserNamesHavingmorethan600000Friends();
			response.sendRedirect("MoreFreinds.html");
			break;
		case 6:
			TimeQuery();
		...
		case 8:
			GamesQuery();
			response.sendRedirect("TopGames.html");
			break;
		case 9:
			TweetStatusQuery();
			response.sendRedirect("tweet_status_analysis.html");
...
//			}
			...
			{
	        	URL url=getClass().getResource("Output1.txt");
...
//				}
			...
			public void Top8BackgroundImageColor()
			{
				URL url=getClass().getResource("Output1.txt");
	            String pathToFile = url.toString();
				SparkConf conf = new SparkConf().setAppName("User mining").setMaster("local[*]");
...
//				}
		  ...
		  private void nbTweetByBackgroundImageColor(JavaSQLContext sqlContext) 
		  {
			...
			public  void UserNamesHavingmorethan600000Friends()
			{
				URL url=getClass().getResource("Output1.txt");
	            String pathToFile = url.toString();
				 SparkConf conf = new SparkConf().setAppName("User mining").setMaster("local[*]");
...
//		  		}
			 ...
			 private void nbTweetByFriends(JavaSQLContext sqlContext) {
			    ...
			    JavaSchemaRDD count = sqlContext.sql("SELECT DISTINCT user.screen_name, user.friends_count AS c FROM tweetTable " +
			    										"WHERE user.friends_count>'300000'" +
			    		                                  "order by c" ); 
...
//		 		}
			 ...
			 private void nbTweetByGamesQuery(JavaSQLContext sqlContext) {
